...
import {
  Button, Input, Select, Card, PageHeader, EmptyState, Loading, StatusBadge
} from '../components/ui'
  ...
  return (
    <div>
      <PageHeader
        ...
        action={<Button icon={Plus} onClick={openCreate}>Jadwal baru</Button>}
      />
            ...
            <tbody className="divide-y divide-stone-100">
              {items.map((s) => (
                <tr key={s.id} className="hover:bg-stone-50/50 transition">
                  <td className="px-5 py-3 font-medium text-stone-900">{s.customer_name}</td>
                  <td className="px-5 py-3 text-stone-600">{s.user_name}</td>
                  <td className="px-5 py-3 text-xs text-stone-400">{s.scheduled_at}</td>
                  ...
                  <td className="px-5 py-3 text-right">
                    ...
                    >
                      {['pending', 'on_route', 'done', 'cancelled'].map((st) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-xl w-full max-w-lg shadow-xl">
            <div className="px-6 py-4 border-b border-stone-200 flex items-center justify-between">
              <h2 className="text-sm font-semibold text-stone-900">Jadwal baru</h2>
              <button
            ...
            <form onSubmit={submit} className="p-6 space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                <Button type="submit" className="flex-1">Buat jadwal</Button>
                <Button
                  type="button"
                  variant="secondary"
                  onClick={() => setShowForm(false)}
