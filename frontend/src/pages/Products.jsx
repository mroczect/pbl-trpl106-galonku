...
import {
  Button, Input, Select, Card, Badge, PageHeader, EmptyState, Loading
} from '../components/ui'
...
export default function Products() {
  ...
  const [form, setForm] = useState({ sku: '', name: '', category: 'galon', price: '', stock: '' })
  ...
  return (
    <div>
      <PageHeader
        ...
        action={isAdmin && <Button icon={Plus} onClick={openCreate}>Tambah produk</Button>}
      />
        ...
        {loading ? <Loading /> : items.length === 0 ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-stone-100">
              {items.map((p) => (
                <tr key={p.id} className={`hover:bg-stone-50/50 transition ${!p.is_active ? 'opacity-40' : ''}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/40 backdrop-blur-sm">
          <div className="bg-white rounded-xl w-full max-w-lg shadow-xl">
            ...
            <form onSubmit={submit} className="p-6 space-y-4">
              ...
              <Input label="Nama produk" required value={form.name}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Input label="Harga" type="number" required value={form.price}
                  onChange={(e) => setForm({ ...form, price: e.target.value })} />
              </div>
              <Input label="Stok" type="number" value={form.stock}
